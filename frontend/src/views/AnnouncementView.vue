<template>
  <div class="content-enter">
    <h2 style="font-size:20px;margin-bottom:20px">网站公告</h2>

    <div class="announcement-list">
      <!-- 在这里添加新的公告条目，最新的放在最上面 -->
      <!-- 格式：复制一条 .announce-item，修改日期和内容 -->
      <div class="announce-item card">
      <div class="announce-date">2026-09-25</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.66优化版</p>
        <ul>
          <li> 优化了私信功能查询会话的速度</li>
          <li> 优化了前端的错误提示</li>
          <li> 修复了多窗口同时登录时token不唯一问题</li>
          <li> 细化了异常的分流处理</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-09-23</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.65优化版</p>
        <ul>
          <li> 修复了我大脑缺失导致更新传错服务器位置导致很多BUG的问题</li>
          <li> 优化了商品查询速度</li>
          <li> 修复了修改密码后无法登录的问题</li>
          <li> 加入了修改密码的功能</li>
          <li> 修复了个人信息中无法正常显示注册时间的问题</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-09-14~2026-09-21</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.6优化版</p>
        <ul>
          <li> 修复了同时在线人数过多时会导致数据库连接崩溃的问题</li>
          <li> 优化了数据库查询,提高用户访问速度</li>
          <li> 修复了AI审核使全站等待的问题</li>
          <li> 修复了用户信息会在接口返回的问题</li>
          <li> 优化了响应速度</li>
          <li> 修复了用户误删正常帖子的问题</li>
          <li> 修复了图片上传会因为图片名称导致异常的问题</li>
          <li> 修复了翻页功能的异常</li>
          <li> 修复了用户可以给自己管理员权限的BUG</li>
          <li> 修复了帖子置顶的问题</li>
          <li> 修复删除用户时的连锁BUG</li>
          <li> 修复了用户信息会在暴露的问题</li>
          <li> 修复了实时通讯中校验形同虚设的问题</li>
          <li> 修复了部分逻辑重复的问题</li>
          <li> 增加了帖子上传图片的功能</li>
          <li> 修复了无法上传多张图片的问题</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-09-4</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.26优化版</p>
        <ul>
          <li> 增加token轮换制</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-09-4</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.26优化版</p>
        <ul>
          <li> 增加token轮换制</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-09-3</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.24优化版</p>
        <ul>
          <li> token改为哈希后存放</li>
          <li> 增强用户信息安全性</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-09-2</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.2优化版</p>
        <ul>
          <li> 全站报错将会记录进日志</li>
          <li> 重构了全部异常处理逻辑</li>
          <li> 路由层的返回字典统一</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-08-29</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.18优化版</p>
        <ul>
          <li> 增加图片上传的校验</li>
          <li> 修复了图片上传失败不提示的情况</li>
          <li> 修复了图片上传会阻塞全站的隐患</li>
          <li> 修复了图片可能会撑爆服务器内存的隐患</li>
          <li> 优化了前端界面</li>
          <li> 修复了安全优化后导致前端等级显示错误的问题</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-08-28</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.16优化版</p>
        <ul>
          <li> 增强数据库连接稳定性与性能</li>
          <li> 修复了不同时区会造成token时间误差的BUG</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-08-27</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.12优化版</p>
        <ul>
          <li> 限制非管理员查询功能，增强安全性</li>
          <li> 解决了登出时账号缓存的问题</li>
          <li> 强化了登录过程的安全性，可防御时序攻击</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-08-26</div>
        <h3>论坛环境安全优化</h3>
        <p>郑财校园墙1.1公测版</p>
        <ul>
          <li> 新增校验模型，增加响应安全性</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-08-20</div>
        <h3>论坛更新与优化</h3>
        <p>郑财校园墙1.02公测版</p>
        <ul>
          <li> 更换网站风格</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-08-17</div>
        <h3>论坛开源与公测</h3>
        <p>郑财校园墙1.0公测版</p>
        <ul>
          <li> 私信功能改为使用websocket实时通讯</li>
          <li> 使退出账号时进行浏览器缓存清理</li>
          <li> 移除了轮询法私信的定时访问</li>
          <li> 基础功能完结撒花</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>


      <div class="announce-item card">
        <div class="announce-date">2026-08-14</div>
        <h3>论坛业务拓展</h3>
        <p>郑财校园墙0.6内测版</p>
        <ul>
          <li> 修复了帖子编辑时间显示bug</li>
          <li> 增加了AI审核员，它会理性删除被举报的帖子</li>
          <li> 修复了用户显示非本人帖子举报按钮为删除按钮的bug</li>
          <li> 尝试解决了浏览器缓存可能会导致账号数据错误的问题</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-08-3</div>
        <h3>论坛业务拓展</h3>
        <p>郑财校园墙0.5内测版</p>
        <ul>
          <li> 修复了邮箱注册的准确性</li>
          <li> 新增了私信功能</li>
          <li> 小站进行了全面装修</li>
          <li> 图片上传功能改为双重验证</li>
          <li> 图片上传后将会被压缩</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-07-22</div>
        <h3>论坛业务拓展</h3>
        <p>郑财校园墙0.43内测版</p>
        <ul>
          <li> 新增举报功能，将直接通知管理员</li>
          <li> 修复了删除帖子功能，现已可以正常使用</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-07-15</div>
        <h3>论坛业务拓展</h3>
        <p>郑财校园墙0.40内测版</p>
        <ul>
          <li>交易模块新增图片功能</li>
          <li>用户可以标记交易情况</li>
          <li>管理员可以删除所有内容了</li>
          <li>修复了迁移链问题</li>
          <li>根据大家的建议新增了等级系统</li>
          <li>增加了等级标识和管理员标识</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-07-6</div>
        <h3>论坛业务拓展</h3>
        <p>郑财校园墙0.30内测版</p>
        <ul>
          <li>新增交易模块</li>
          <li>可以在商品下进行评论了</li>
        </ul>
        <p>如有问题反馈至3511687086@qq.com</p>
      </div>

      <div class="announce-item card">
        <div class="announce-date">2026-06-18</div>
        <h3>论坛正式上线</h3>
        <p>郑财校园墙0.21内测版</p>
        <ul>
          <li>修复了浏览量显示问题</li>
          <li>修复了通知功能的使用</li>
          <li>增加了论坛通知</li>
          <li>爆金币买了个域名</li>
          <li>炫了个薄荷冰淇淋</li>
        </ul>
        <p>如有问题或建议，欢迎在站内发帖反馈。祝大家使用愉快！</p>
      </div>

      <!-- 添加更多公告：复制上面的 .announce-item 块 -->
    </div>
  </div>
</template>

<script setup>
// 公告内容直接写在 template 里
// 修改后本地 build 再部署即可更新
</script>

<style scoped>
.announcement-list {
  max-width: 700px;
}
.announce-item {
  margin-bottom: 16px;
}
.announce-item:last-child {
  margin-bottom: 0;
}
.announce-date {
  font-size: 12px;
  color: var(--text-muted);
  margin-bottom: 6px;
}
.announce-item h3 {
  font-size: 16px;
  margin-bottom: 8px;
  color: var(--primary);
}
.announce-item p {
  font-size: 14px;
  line-height: 1.7;
  color: var(--text-secondary);
  margin-bottom: 6px;
}
.announce-item ul {
  margin: 6px 0 10px 20px;
  font-size: 14px;
  color: var(--text-secondary);
  line-height: 1.8;
}
.announce-item ul li {
  margin-bottom: 2px;
}

@media (max-width: 768px) {
  .announce-item h3 {
    font-size: 15px;
  }
  .announce-item p,
  .announce-item ul {
    font-size: 13px;
  }
}
</style>
